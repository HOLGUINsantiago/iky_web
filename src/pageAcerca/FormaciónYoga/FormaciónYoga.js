import React from "react";
import "./FormaciónYoga.css";
import photo1 from "../../assets/photoPageRipey/FormacionYoga/431A0466.jpg";
import photo2 from "../../assets/photoPageRipey/FormacionYoga/431A0527.jpg";

const FormaciónYoga = () => {
  return (
    <div>
      <p className="text-FormaciónYoga">
        <img src={photo1} alt="Hatha Yoga" className="img-right" />
        C.R. Kan inició su aprendizaje de yoga en octubre de 1993, cuando
        cursaba su último año de estudio de leyes en la Universidad Externado de
        Colombia, mientras realizaba su práctica de consultorio jurídico para la
        Presidencia de la República ante la Corte Constitucional en Bogotá.
        <br />
        <br />
        La exigencia académica y profesional le requirió una práctica física que
        además le aportara relajación y descanso, optando por el hatha yoga. La
        mencionada disciplina le resultó difícil y agresiva, a lo cual se sumó
        su lenguaje y simbología extraña y de tinte religioso. Estas últimas
        características resultaban ajenas a su temperamento escéptico, crítico,
        ateísta y laicista que siempre lo han acompañado, y que desarrolló
        durante sus estudios escolares en el Liceo Francés de Cali, su ciudad
        natal.
        <br />
        <br />
        Sin embargo, continuó asistiendo a clases de hatha yoga esporádicamente,
        principalmente con el profesor Asdrúal Lozano, hasta que, en el año
        2000, siendo inspector de policía en el Municipio de La Calera, al
        nororiente de Bogotá, conoció al mamo wiwa don Ramón Gil, quien lo
        inspiró a iniciar un camino espiritual.
        <br />
        <br />
        Retomó la práctica de hatha yoga con la maestra Beatriz Rodríguez de la
        Gran Fraternidad Universal, quien lo inició en técnicas más profundas y
        le evidenció la escuela de Satyananda Yoga, quien posteriormente se
        convertiría en su gurú. También estudió varios meses entre el 2000 y el
        2001 con el profesor Goranga Lila en el templo de la secta Hare Krishna
        en Bogotá.
        <br />
        <br />
        Durante 2004, también en Bogotá, fue formado en tantra kriya yoga por la
        maestra kaula Tutare Devi Dasi, quien lo inició como maestro de hatha
        yoga en julio de ese mismo año.
        <br />
        <br />
        A partir del año 2005 comenzó a practicar Satyananda Yoga con los
        talleres tomados en Cali dictados por Swami Darmadeva.
        <br />
        <br />
        <img src={photo2} alt="Mamo Wiwa" className="img-left" />
        En 2006 fue iniciado en meditación Ishaia por los maestros Bhairavinanda
        Ishaia y Balaram Ishaia en su ciudad natal.
        <br />
        <br />
        En febrero de 2007 se certificó como instructor de yoga integral con
        Yogui Maurya y como instructor de power yoga con Marcela Larrosa en la
        Escuela Yogakai en Buenos Aires, Argentina. En febrero de 2009 se
        certificó en la misma escuela como maestro de yoga clásico, con posgrado
        en yoga para niños y para maternas. Al mismo tiempo se certificó con el
        maestro Aníbal Bonelli como instructor de taichi y chikung en la escuela
        Naturaleza Tai de Argentina.
        <br />
        <br />
        El 21 de mayo de 2012 en Bogotá, recibió diksha patra de manos de la
        gurú Swami Satyasangananda Saraswati de la Bihar School of Yoga de
        India.
        <br />
        <br />
        En diciembre de 2019 aprobó el curso de un año de Estudios Yóguicos 1 de
        la Bihar School of Yoga de India con su representante en Colombia la
        Academia de Yoga Satyananda de Bogotá. En el año 2020 continuó en la
        misma escuela aprobando en diciembre Estudios Yóguicos 2 y en enero de
        2024 inició el nivel de profesorado con la misma institución, después de
        20 años de estar ejerciendo con constancia y permanencia como maestro de
        yoga.
        <br />
        <br />
        Durante sus más de 30 años de estudio y práctica del yoga ha tomado
        diversas clases, retiros, talleres y cursos con diferentes profesores de
        variadas escuelas de yoga, tanto en Colombia como en otros países.
      </p>
    </div>
  );
};

export default FormaciónYoga;
