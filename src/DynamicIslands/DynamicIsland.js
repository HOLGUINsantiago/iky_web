...
const IslaDinamica = ({ sections, sectionRefs }) => {
    ...
    const [isRecomendacionesMenuOpen, setIsRecomendacionesMenuOpen] = useState(false);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (currentRef) {
                ...
                const progressInSection = Math.min(Math.max((progressLine - top) / height, 0), 1) * 100;
                setProgress(progressInSection);

                const updatedRecommendations = sections[currentSection - 1].recommendations.filter((rec) => {
                    const recProgress = rec.porcentaje || 0;
                    return recProgress <= progressInSection + 20 && recProgress >= progressInSection - 20;
                });

                if (JSON.stringify(currentRecommendations) !== JSON.stringify(updatedRecommendations)) {
    ...
    useEffect(() => {
        const adjustDimensions = () => {
            if (leftRef.current && rightRef.current && !isSectionMenuOpen && !isRecomendacionesMenuOpen) {
    ...
    }, [isSectionMenuOpen, sections, showSectionName, sectionName, currentRecommendations, showRecommendationTitle, isRecomendacionesMenuOpen]);


    const handleMouseEnterSection = () => {
        if (!isSectionMenuOpen)
            setIslaWidth(islaRef.current.scrollWidth);
    ...
    return (
        <div className='box'>
            <div className={`isla-dinamica ${isSectionMenuOpen ? 'menu-open' : ''} ${isRecomendacionesMenuOpen ? 'recomendaciones-open' : ''} `} ref={islaRef}>
                <div className="shape">
                    {!isSectionMenuOpen && (
                        <div className="isla-dinamica-left" ref={leftRef} onMouseEnter={handleMouseEnterRecomendaciones} onMouseLeave={handleMouseLeaveRecomendaciones}>
                            <div className="isla-dinamica-left-content">
                                {sections[currentSection - 1].recommendations.length > 0 && (
                                    <div className="recommendations">
                                        {!showRecommendationTitle && currentRecommendations.length > 1 && !isRecomendacionesMenuOpen && (
                                            <p>{currentRecommendations.length} recomendaciones</p>
                                        )}
                                        {!showRecommendationTitle && currentRecommendations.length === 1 && (
                                            <p>{currentRecommendations[0].title}</p>
                                        )}
                                        {!showRecommendationTitle && currentRecommendations.length === 0 && (
                                        ...
                                        {showRecommendationTitle && (
                                            <p>
                                                {currentRecommendations.map((rec, index) => (
                                                    <span key={index}>
                                                        <a href={rec.url} className="no-link">{rec.title}</a>
                                ...
                                {currentRecommendations.length != 0 && isRecomendacionesMenuOpen && (
                                    <div className="menu" style={{ minWidth: `${islaWidth}px` }} ref={menuRef}>
                                        <ul>
                                            {currentRecommendations.map((section) => (
                                                <li key={section.title}>
                                                    <a onClick={() => {
                                                        section.allowed ? setVentanaRecActiva(section.url)
                                ...
                                {currentRecommendations.length != 0 && ventanaRecActiva != null && (
                                    <div className="menu" style={{ minWidth: `${islaWidth}px` }} ref={menuRef}>
                                        <iframe
                                            src={ventanaRecActiva}
                                            style={{ width: '60vw', height: '35vw', border: 'none' }}
                    ...
                    <div className="isla-dinamica-middle">
                        {!isSectionMenuOpen && (
                            <button className="btn" onClick={handleMenuPress()}><i className="fa fa-bars"></i></button>
                        )}
                    </div>
                    <div className={`isla-dinamica-right ${isSectionMenuOpen ? 'menu-open' : ''}`} ref={rightRef} onMouseEnter={handleMouseEnterSection} onMouseLeave={handleMouseLeaveSection}>
                            ...
                            <div className="menu" style={{ minWidth: `${islaWidth}px` }} ref={menuRef}>
                                <ul>
                                    {sections.map((section) => (
                                        <li key={section.id}>
                                            <a href={`#section-${section.id}`} onClick={() => {
                                                setCurrentSection(section.id);
                                                sectionRefs.current[section.id - 1].scrollIntoView({ behavior: 'smooth' });
                                                setIsSectionMenuOpen(false);
                                            }}>
                                                {section.name}
                                            </a>
